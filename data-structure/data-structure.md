...
- 이진탐색트리와 달리 편향이 발생하지 않는 이유: 이진탐색트리와 달리 편향이 발생하지 않는 이유는 힙은 완전 이진 트리의 구조를 유지하고 있기 때문이다. 삽입이나 삭제 연산 후에도 왼쪽부터 꽉챠있는 구조를 유지하기에 편향이 발생하지 않는다.
...
### **7. BBST (Balanced Binary Search Tree) 와, 그 종류에 대해 설명해 주세요.**
  ...
  - red-black tree:
    - 트리의 모든 노드가 red 또는 black이다.
    - 색상을 관리함으로써 삽입, 삭제, 탐색 연산의 시간복잡도가 O(logn)이다.
    - root 노드는 무조건 블랙이다.
    - 모든 리프 노드는 블랙이다.
    - 루트 노드에서 단말 노드까지의 블랙의 개수는 항상 같다.
    - 레드 노드의 자식은 모두 블랙이다.
    - 레드-블랙 트리에서 새로운 노드를 삽입할 때 새로운 노드는 항상 레드로 삽입한다.
